...
// 1. 更新 layoutSankey 支持在 requests 维度下自动收敛至 Layer 3 (状态门)
...
if (sIdx === -1 || eIdx === -1) {
  console.error('layoutSankey bounds not found in sidebar.ts');
  process.exit(1);
}
...
const newLayoutSankey = `  function layoutSankey(data, dimension, width) {
    ...
    ((data || {}).nodes || []).forEach((node) => {
      ...
      // 在 requests 次数维度下，请求的生命周期在状态层 (Layer 3) 完美终止，忽略下游 Token 分流节点
      if (isReq && layer > 3) return;
      byId.set(node.id, {
...
// 2. 更新 renderSankey：根据维度动态更新分流桑基图副标题提示 (sankeySub) 与容器宽度
...
const newRenderSankeyTop = `  function renderSankey(data) {
    ...
    if (subEl) {
      subEl.textContent = sankeyDim === 'tokens' ? '渠道 → 凭证 → 模型 → 状态 → Token分类 → 上下文构成' : '渠道 → 凭证 → 模型 → 状态 (请求闭环)';
    }
    box.style.minWidth = sankeyDim === 'tokens' ? '1080px' : '720px';
    const layout = layoutSankey(data, sankeyDim, box.clientWidth);`;
text = text.replace(oldRenderSankeyTop, newRenderSankeyTop);

// 3. 在 HTML 模板中给副标题添加 id="sankeySub"
const oldMutedSub = `<div class="muted" style="font-size:10.5px;margin-top:2px;">渠道 &rarr; 凭证 &rarr; 模型 &rarr; 状态</div>`;
const newMutedSub = `<div class="muted" id="sankeySub" style="font-size:10.5px;margin-top:2px;">渠道 &rarr; 凭证 &rarr; 模型 &rarr; 状态</div>`;
text = text.replace(oldMutedSub, newMutedSub);

fs.writeFileSync(filePath, text, 'utf-8');
console.log('sidebar.ts successfully updated with request dimension convergence & accurate integer conservation!');
